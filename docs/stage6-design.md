...
> 本書を Stage 6 の Wave 構成・学習型ラスボス・実装設計の正本とする。
> Wave 1〜4 の詳細ステータスは未確定。
...
**設計方針**:
- 新しい構文や新システムは導入しない
- Wave 1〜4 は Stage 1〜4 の代表ボスを強化版で再登場させる
- Wave 5 は Stage 1〜5 の行動履歴に適応する学習型ラスボスにする
...
Wave 1〜4 の詳細ステータスと NPC の扱いは別途決定する。Wave 5 のラスボス適応ロジックは本書の内容で確定とする。
...
集計は **Stage 1〜5 のすべてのクリア時** に蓄積される。バトル終了時（victory 判定後）に `actionHistory` に集計値を加算し、既存の `useGameStore.ts` 内の `recordMagicUse`・`recordAction` を活用する。
...
| 適応設定 | 実装箇所 |
|----------|----------|
| `resistMagics` | `multiBattle.ts` の単属性ダメージ計算 |
| `comboDamageMultiplier` | `multiBattle.ts` の合体魔法ダメージ計算 |
| `chargeInterval` | `multiBattle.ts` のチャージサイクル処理 |
...
interface EnemyData {
  // ... 既存フィールド ...
  healAllies?: { amount: number }; // Stage 5: ヒーラー
...
`WaveData` は Stage 4 までに確立した `simultaneous` / `stateGimmickOverride` の構造で足りるため、Stage 6 では拡張しない。
...
| ラスボスの適応セリフ表示 | `BattleAnimator.tsx` または専用コンポーネント |
| 適応中バッジ | HP バー横などに表示してもよい（任意） |
