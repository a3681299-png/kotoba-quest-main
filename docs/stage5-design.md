...
> 詳細仕様は `docs/stage5-6-spec.md §1` が正本。本書はエンジン設計の観点から補足する。
...
**学習目標**: Stage 1〜4 の要素（順次実行・繰り返し・条件分岐・NPC・合体魔法・状態変化・文字数制限）を組み合わせた実践問題。新構文は導入しない。
...
Stage 6 の学習型ラスボスのために、Stage 5 のバトル終了時から `ActionHistory` への集計を開始する。
