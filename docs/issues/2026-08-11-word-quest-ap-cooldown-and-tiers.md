...
> **⚠️ 2026-08-14 追記**: 本docで計画していた「動詞ごとのクールタイム（CT）」システムは、実際にPart AのAP基盤を実装して動かしたところ、AP不足による制約と役割が重複し体感バランスも取りづらかったため**全廃**した。AP（行動値）のみで行動回数を制約する方式に一本化している。CT前提の記述（動詞別`apCost`/`cooldown`表、CT関連のデータ層・ロジック層・UI層の変更点）は古い設計として参照時に注意すること。唯一の例外として、echo-moth（反響の蛾）には「同じ動詞を2回連続で使用できない」という専用ルールを設けている（詳細は[エネミーギミック再調整doc](./2026-08-11-word-quest-enemy-gimmick-redesign.md)を参照）。Part B/C（Tier1新規敵・三層構成）は未着手のまま。
